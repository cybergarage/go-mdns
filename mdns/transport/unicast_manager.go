...
func (mgr *UnicastManager) SetMessageProcessor(processor dns.MessageProcessor) {
...
func (mgr *UnicastManager) lookupAppropriateServerForInterface(ifi *net.Interface) (*UnicastServer, error) {
	...
	for _, server := range mgr.Servers {
		...
		listenIfi, err := server.UDPSocket.ListenInterface()
		if err != nil {
			continue
		}
		if listenIfi == ifi {
...
func (mgr *UnicastManager) SendMessage(addr string, port int, msg dns.Message) (int, error) {
	var lastErr error
	for _, server := range mgr.Servers {
...
func (mgr *UnicastManager) AnnounceMessage(msg dns.Message) error {
	var lastErr error
	for _, server := range mgr.Servers {
...
func (mgr *UnicastManager) PostMessage(addr string, port int, reqMsg dns.Message) (dns.Message, error) {
	if !mgr.TCPEnabled() {
